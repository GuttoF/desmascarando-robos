...
class MLPipeline:
    ...
    def build_pipeline(
        ...
        if hashing_list:
            logging.info("Hashing encoding features: %s", hashing_list)
            preprocessing_steps.append(("hashing", HashingEncoder()))

        if ordinal_list:
            logging.info("Ordinal encoding features: %s", ordinal_list)
            preprocessing_steps.append(("ordinal", OrdinalEncoder()))
    ...
    def train_and_evaluate_models(self, X_test, y_test, threshold=0.5):
        ...
        for pipeline in self.pipelines:
            ...
            scores = {
                ...
                "Brier Score Loss": brier_score_loss(y_test, y_probs),
            }
